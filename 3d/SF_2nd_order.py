...
def str_function_gpu(Vx, Vy, Vz, Ix, Iy, Iz, l_cap_x, l_cap_y, l_cap_z, S_array, S_u_r_array, S_ux_array, S_uy_array, S_uz_array):
    ...
    for m in range(N):
        ...
        S_array[Ix[m], Iy[m], Iz[m]] = cp.mean(diff_magnitude_sqr[:, :, :])
        S_u_r_array[Ix[m], Iy[m], Iz[m]] = cp.mean((del_u[:, :, :]*l_cap_x[m] + del_v[:, :, :]*l_cap_y[m] + del_w[:, :, :]*l_cap_z[m])**2)

        S_ux_array[Ix[m], Iy[m], Iz[m]] = cp.mean((del_u[:, :, :]*l_cap_x[m])**2)
        S_uy_array[Ix[m], Iy[m], Iz[m]] = cp.mean((del_v[:, :, :]*l_cap_y[m])**2)
        S_uz_array[Ix[m], Iy[m], Iz[m]] = cp.mean((del_w[:, :, :]*l_cap_z[m])**2)
